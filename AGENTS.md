...
你在接不同公司的项目来赚钱。目前活跃的有两个：
1. **趣学洋葱**（教育培训，K12 家教 + 雅思语培）— 内部管理系统，前端 onion-dashboard + 后端 onion-agent
2. **友联**（医疗器械、医用耗材、保健食品及日用品的批发与分销）— AI驱动的全渠道商业智能系统，前端 union-dashboard + 后端 union-agent。体量惊人：8.8万客户、1600供应商、1.4万SKU、251万条销售记录，对接金蝶/旺店通/mini橙/顺丰四个外部系统。
...
├── onion-dashboard/        ← 趣学洋葱前端（教育项目前端操作手册）
├── onion-agent/            ← 趣学洋葱后端（教育项目后端操作手册）
├── union-dashboard/        ← 友联前端（商业引擎·指挥大脑）
├── union-agent/            ← 友联后端（商业引擎·执行引擎）
└── （未来更多项目...）
```
