...
| 配置项 | 必填 | 说明 | 示例 |
|--------|------|------|------|
| `BRAIN_PROFILE` | ✅ | 当前 profile 名 | `your_name` |
| `BRAIN_DATABASE_URI` | ✅ | Supabase PostgreSQL 连接串 | `postgresql://postgres.<项目ID>:密码@...pooler.supabase.com:5432/postgres` |
| `OPENROUTER_API_KEY` | 可选 | OpenRouter API Key（启用 LLM + 语义搜索） | `sk-or-...` |
| `LLM_MODEL` | 可选 | 文本 LLM（默认 `google/gemini-2.5-flash`） | |
| `LLM_MODEL_OMNI` | 可选 | 多模态 LLM（默认 `xiaomi/mimo-v2-omni`） | |

**最小配置**：只需 `BRAIN_PROFILE` + `BRAIN_DATABASE_URI`，其余全部自动推导。
...
- 人类搭档提到某个行业/话题/技术，需要了解背景 → `learn` 或 `search`
- 需要研究竞品、市场趋势 → `search --save`
- 人类搭档分享了一个 URL，想了解内容 → `learn`
- AI 主动为搭档准备行业洞察 → `search`
...
brain_db.py stats
brain_db.py dump --kind memory --subject $BRAIN_PROFILE
```
...
写入时自动生成两级摘要（需 LLM key）：
- **L0**（一句话摘要 ~30字）：`find` 默认输出，快速扫描
- **L1**（关键要点 ~200字）：`find` 默认输出，辅助决策
- **L2**（全文）：`find --detail` 输出，深度阅读

```bash
brain_db.py find --kind memory                # 输出 L0/L1 摘要
brain_db.py find --kind memory --detail       # 输出全文（L2）
brain_db.py backfill-layers --limit 100       # 批量为已有记录生成 L0/L1
brain_db.py backfill-layers --kind knowledge  # 只处理知识类
```
...
| `xiaomi/mimo-v2-omni` | 多模态：图片/视频/音频理解 | OpenRouter |
| `openai/text-embedding-3-small` | Embedding 向量（语义搜索） | OpenRouter |

**一个 OpenRouter API Key 驱动全部功能。** 存储在 `brain.secrets` 表中。所有 HTTP 调用使用 Python stdlib `urllib`，**零外部依赖**。

依赖链：
- learn/search → Firecrawl CLI + LLM (gemini-2.5-flash)
- see → LLM (mimo-v2-omni，多模态)
- reflect/digest/auto-link → LLM (gemini-2.5-flash)
- embedding → OpenRouter (text-embedding-3-small，直连)
...
# 2. 可选回忆
brain_db.py find --kind memory --subject $BRAIN_PROFILE --limit 10
...
```bash
brain_db.py add "搭档本周重点是把某功能打磨一轮" \
  --kind memory --subject $BRAIN_PROFILE \
...
```
brain_db.py (本地)
  ↓ HTTP POST (Bearer OPENROUTER_API_KEY，直连，无中间层)
OpenRouter API → openai/text-embedding-3-small
  ↓ 返回 1536 维向量
brain_db.py → 写入 brain.entries.embedding
```

**无需部署 Edge Function。** Embedding 直接走 OpenRouter，和 LLM 共用同一个 API Key。
...
通过 Cursor 的 `sessionEnd` hook，每次对话结束时自动从 transcript 提取记忆并写入数据库。

**工作原理**：
```
对话结束 → Cursor 触发 sessionEnd hook → session_end.py 读取 transcript
→ LLM 提取记忆（memory/event/convo） → 写入 brain.entries（tag: auto-session）
```

**配置**：`~/.cursor/hooks.json` 注册了 sessionEnd hook，指向 `session_end.py`。

**日志**：`~/.cursor/hooks/session-end.log`

**提取规则**：
- 关于人类搭档的新认知 → `kind=memory`
- 重要里程碑或决策 → `kind=event`
- 对话摘要（每次必有） → `kind=convo`
- 纯技术操作不提取，只生成 convo 摘要
- 所有自动提取的记录带 `auto-session` tag，confidence=0.7

**与手动记忆的关系**：自动提取是兜底，不替代对话中的主动 `brain_db.py add`。AI 在对话中发现重要信息仍应立即写入（confidence 更高）。自动提取负责捞遗漏。
...
- 语义检索和 LLM 功能共用 `openrouter_api_key`，配一个 key 全部启用
